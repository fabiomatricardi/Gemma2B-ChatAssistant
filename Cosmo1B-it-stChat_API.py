...
st.set_page_config(
    page_title="Your own LocalGPT with 🛸 cosmo-1b",
    page_icon="🛸",
    layout="wide")

# Create a header element
st.header("Your own LocalGPT with 🛸 cosmo-1b")
st.markdown("#### :green[*cosmo-1b Q4 GGUF - the best 1B model?*]")
...
## Logger file
    ...
    writehistory(st.session_state.logfilename,f'Your own LocalGPT with 🛸 cosmo-1b\n---\n🧠🫡: You are a helpful assistant.')    
...
# CREATE THE SIDEBAR
with st.sidebar:
    st.markdown("""### 🛸 cosmo-1B
- ConversationBuffer+Limiter
- Real streaming output
- HyperParameters""", unsafe_allow_html=True)
    mytokens = st.markdown(f"""**Context turns**: {st.session_state.len_context}  **n_ctx**: 2048""")
